      ...
      for (let i = 0; i < filteredAssets.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async (asset) => {
          try {
            ...
            // Filter out empty candle responses
            const validCandles = candles && Array.isArray(candles) ? candles : [];
            const minCandles = config.testnet ? 10 : 100;
            if (validCandles.length < minCandles) return;

            // 3. Performance Metrics
            const prices = validCandles.map((c: any) => parseFloat(c.c));
