...
// Fix OrderBook.tsx
let obCode = fs.readFileSync('src/components/OrderBook.tsx', 'utf-8');
obCode = obCode.replace(/key=\\{\\\`ask-\\\$\\{i\\}\\\`\\}/g, 'key={`ask-${i}`}');
obCode = obCode.replace(/key=\\{\\\`bid-\\\$\\{i\\}\\\`\\}/g, 'key={`bid-${i}`}');
obCode = obCode.replace(/width: \\\`\\\$\\{/g, 'width: `${');
obCode = obCode.replace(/\\}%\` /g, '}%` ');
obCode = obCode.replace(/\\`/g, '`');
obCode = obCode.replace(/\\\$/g, '$');
obCode = obCode.replace(/\\{/g, '{');
obCode = obCode.replace(/\\}/g, '}');
fs.writeFileSync('src/components/OrderBook.tsx', obCode);

// The above regexes might be messy, I will just rewrite OrderBook.tsx correctly since it's small.
const obContent = `import React, { useEffect, useState, useRef } from 'react';
...
fs.writeFileSync('src/components/OrderBook.tsx', obContent);

// Fix App.tsx Duplicate Title
let appCode = fs.readFileSync('src/App.tsx', 'utf-8');
appCode = appCode.replace(/title="Halt the running DCA Engine" title="Halt the running Grid Master" /g, '');
